...
## 2026-05-07 - Improve batch empty state with actionable CTA
**Learning:** Conversational bot UI empty states (like 'batch empty') must include clear, context-specific call-to-actions (CTAs) instead of reusing generalized strings, guiding the user on what to do next. For example, suggesting sending a media group for batch forwarding.
**Action:** Always provide specific, actionable instructions in empty states rather than just stating there is no data, avoiding the reuse of generic messages across different contexts.
