...
## 2026-05-15 - Refactor SmartFailover executeBatch to Async Worker Pool
**Learning:** The bounded async worker pool using `Array.from` with a `while` loop avoids dynamic array sizing and connection pool exhaustion when dealing with multiple sequential operations to the same or unbounded lengths, without sacrificing return order matching.
**Action:** Use native async worker pools for large batch iterations natively and replace bounded dynamic length array mutations inside `.map()` with `new Array(len)` when strict performance is needed.
