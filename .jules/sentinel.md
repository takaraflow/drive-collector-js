...
## 2026-05-25 - Prevent Predictability in Stream Finalization Tokens
**Vulnerability:** Used insecure `Math.random()` to generate the `finalizeToken` for resumable stream uploads.
**Learning:** Just like with distributed lock versions, `Math.random()` is not cryptographically secure and produces predictable identifiers. This could theoretically allow an attacker to guess a stream's finalization token and forcibly complete or cancel the stream if they also know the `taskId`.
**Prevention:** Always use cryptographically secure PRNGs such as `crypto.randomUUID()` or `crypto.randomBytes(N).toString('hex')` for security-sensitive tokens, including authentication and upload session identifiers.
