...
## 2025-05-24 - Prevent Predictability in Finalization Tokens
**Vulnerability:** Used insecure `Math.random()` to generate finalization tokens in StreamTransferService.
**Learning:** `Math.random()` is not cryptographically secure, meaning generated identifiers are predictable. This predictability can lead to an attacker guessing the token and finalizing/tampering with upload streams maliciously.
**Prevention:** Always use cryptographically secure PRNGs like `crypto.randomUUID()` imported from `node:crypto` to generate security-sensitive tokens.
