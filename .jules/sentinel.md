...
## 2026-07-26 - Prevent Predictability in Stream Finalization
**Vulnerability:** Used `Math.random()` to generate the `finalizeToken` for stream transfers.
**Learning:** `Math.random()` is not cryptographically secure, meaning generated identifiers are predictable. This predictability could allow an attacker to guess the token and potentially finalize or abort stream transfers prematurely, bypassing intended security validations.
