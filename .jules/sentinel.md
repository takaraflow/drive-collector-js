...
## 2026-09-04 - Prevent Timing Attacks in Secret Comparison
**Vulnerability:** The application was using the standard strict equality operator (`===`) to compare the incoming `x-instance-secret` header against the expected configured instance secret in `StreamTransferService.js`.
**Learning:** A standard equality operator `===` short-circuits as soon as it finds a non-matching character, resulting in a variable execution time depending on how much of the string matched. This variable timing can be exploited (a timing attack) to guess the secret character by character, bypassing authentication.
**Prevention:** Always use a constant-time comparison function, such as `crypto.timingSafeEqual()` from the `node:crypto` module, when validating passwords, hashes, HMACs, or authentication secrets. Additionally, ensure that inputs have the same length before calling `timingSafeEqual()` (by checking buffer lengths), as the function will throw an exception if the lengths differ, revealing the true length of the expected secret.
