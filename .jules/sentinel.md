...
## $(date +%Y-%m-%d) - Prevent Metadata Exposure of Secrets
**Vulnerability:** Logged the exact length of the D1 authentication token (`this.token?.length`) upon authentication failure.
**Learning:** Logging the exact length of sensitive credentials (like tokens or passwords) can expose metadata that might assist an attacker in guessing or brute-forcing the secret, breaking the "defense in depth" principle.
**Prevention:** Never log the exact length of sensitive credentials. Instead, log their presence or absence (e.g., `token ? 'present' : 'missing'`) to provide debugging context without leaking any metadata.
