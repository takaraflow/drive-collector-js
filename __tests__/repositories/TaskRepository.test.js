...
describe('TaskRepository', () => {
    ...
    describe('getQueueOverview', () => {
        ...
        it('should verify SQL query structure and parameters', async () => {
            ...
            expect(mockD1.batch).toHaveBeenCalledWith([
                { sql: expect.stringContaining('GROUP BY status'), params: [] },
                { sql: expect.stringContaining('LIMIT ?'), params: [...TASK_ACTIVE_STATUSES, 15] },
                { sql: expect.stringContaining('LIMIT 5'), params: [...TASK_ACTIVE_STATUSES] }
            ]);
        });
        ...
        it('should use default limit of 10 when not specified', async () => {
            ...
            expect(mockD1.batch).toHaveBeenCalledWith([
                { sql: expect.stringContaining('GROUP BY status'), params: [] },
                { sql: expect.stringContaining('LIMIT ?'), params: [...TASK_ACTIVE_STATUSES, 10] },
                { sql: expect.stringContaining('LIMIT 5'), params: [...TASK_ACTIVE_STATUSES] }
            ]);
        });
    });
    ...
    describe('getUserQueueOverview', () => {
        ...
        it('should filter every query by user id and active statuses', async () => {
            ...
            expect(mockD1.batch).toHaveBeenCalledWith([
                { sql: expect.stringContaining('WHERE user_id = ?'), params: ['user123'] },
                { sql: expect.stringContaining('WHERE user_id = ? AND status IN'), params: ['user123', ...TASK_ACTIVE_STATUSES, 7] },
                { sql: expect.stringContaining('WHERE user_id = ?'), params: ['user123', 7] }
            ]);
        });
        ...
        it('should use default limit for invalid limit values', async () => {
            ...
            expect(mockD1.batch).toHaveBeenCalledWith([
                { sql: expect.stringContaining('WHERE user_id = ?'), params: ['user123'] },
                { sql: expect.stringContaining('LIMIT ?'), params: ['user123', ...TASK_ACTIVE_STATUSES, 10] },
                { sql: expect.stringContaining('LIMIT ?'), params: ['user123', 10] }
            ]);
        });
    });
    ...
    describe('getTasksByStatus', () => {
        ...
        it('should calculate correct offset for page 2', async () => {
            ...
            expect(mockD1.batch).toHaveBeenCalledWith([
                { sql: expect.stringContaining('LIMIT ? OFFSET ?'), params: ['failed', 10, 20] },
                { sql: expect.stringContaining('COUNT(*)'), params: ['failed'] }
            ]);
        });
        ...
        it('should verify SQL query structure', async () => {
            ...
            expect(mockD1.batch).toHaveBeenCalledWith([
                { sql: expect.stringContaining('WHERE status = ?'), params: ['completed', 10, 0] },
                { sql: expect.stringContaining('COUNT(*)'), params: ['completed'] }
            ]);
        });
    });
});
