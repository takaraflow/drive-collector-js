...
vi.mock('http2', () => ({
    createServer: vi.fn((config, handler) => ({
        listen: vi.fn((port, cb) => cb()),
        close: vi.fn(cb => cb())
    })),
    createSecureServer: vi.fn((config, handler) => ({
        listen: vi.fn((port, cb) => cb()),
        close: vi.fn(cb => cb())
    }))
}));
...
describe('lifecycle utilities', () => {
    ...
    describe('httpServer state', () => {
        it('should set and get http server', () => {
    ...
    describe('registerShutdownHooks', () => {
        it('should register all expected shutdown hooks with correct priorities', async () => {
            ...
            // Verify specific hooks were registered with correct priorities
            const expectedHooks = [
                { name: 'logger-flush-before', priority: 5 },
                { name: 'http-server', priority: 10 },
                { name: 'instance-coordinator', priority: 20 },
                { name: 'telegram-client', priority: 30 },
                { name: 'media-group-buffer-persist', priority: 35 },
                { name: 'task-repository', priority: 40 },
                { name: 'distributed-lock', priority: 45 },
                { name: 'media-group-buffer-cleanup', priority: 48 },
                { name: 'cache-service', priority: 50 },
                { name: 'tunnel-service', priority: 55 },
                { name: 'logger-flush-after', priority: 60 },
            ];
        ...
        it('should execute logger flush hooks', async () => {
            vi.useFakeTimers();
        ...
        it('should catch errors when hooks fail', async () => {
            ...
            // We use standard logger in lifecycle.js now
            // But we don't mock error thrown internally - we just check that it resolved without throwing
        ...
        it('should handle distributed lock shutdown gracefully when missing', async () => {
