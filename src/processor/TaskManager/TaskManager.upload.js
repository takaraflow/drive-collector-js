...
export async function uploadTask(task) {
    ...
    try {
        ...
        const uploadResult = await performUploadPhase(task, localPath, fileName, config, ossService, CloudTool, heartbeat, log);
        ...
        if (uploadResult.success) {
            ...
            const validationResult = await validateRemoteFile(this, task, actualFileName, localPath, CloudTool, log);

            await finalizeUploadStatus(this, task, validationResult, info, config, TaskRepository, updateStatus, STRINGS, format);
        } else {
        ...
        await cleanupUploadTask(this, task, localPath, didActivate, instanceCoordinator, log);
    }
}

async function performUploadPhase(task, localPath, fileName, config, ossService, CloudTool, heartbeat, log) {
    let uploadResult;
...
async function validateRemoteFile(context, task, actualFileName, localPath, CloudTool, log) {
    ...
    const isOk = finalRemote && context._isSizeMatch(finalRemote.Size, localSize);
    ...
    return { isOk, localSize, finalRemote, errorMsg };
}

async function finalizeUploadStatus(context, task, validationResult, info, config, TaskRepository, updateStatus, STRINGS, format) {
    const finalStatus = validationResult.isOk ? 'completed' : 'failed';
    await TaskRepository.updateStatus(task.id, finalStatus, validationResult.errorMsg);

    if (task.isGroup) {
        await context._refreshGroupMonitor(task, finalStatus, 0, 0, validationResult.errorMsg);
    } else {
        const fileLink = `tg://openmessage?chat_id=${task.chatId}&message_id=${task.message.id}`;
        const fileNameHtml = `<a href="${fileLink}">${escapeHTML(info.name)}</a>`;
        const baseText = validationResult.isOk
        ...
        const finalMsg = validationResult.isOk ? baseText : `${baseText}\n<code>${escapeHTML(validationResult.errorMsg)}</code>`;
        await updateStatus(task, finalMsg, true);
    }
}

async function cleanupUploadTask(context, task, localPath, didActivate, instanceCoordinator, log) {
    ...
    if (didActivate) {
        context.inFlightTasks.delete(task.id);
    }
