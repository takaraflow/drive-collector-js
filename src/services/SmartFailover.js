...
class SmartFailover {
    ...
    async executeBatch(requestFns, options = {}) {
        ...
        if (parallel) {
            // 并行执行
            // ⚡ Bolt Optimization: Use native async worker pool instead of unbounded Promise.allSettled(array.map)
            const concurrency = options.concurrency || len;
            let currentIndex = 0;

            const workers = Array.from({ length: Math.min(concurrency, len) }, async () => {
