...
class SmartFailover {
    ...
    async executeBatch(requestFns, options = {}) {
        ...
        if (parallel) {
            // 并行执行
            // ⚡ Bolt Optimization: Use native async worker pool instead of unbounded map with Promise.allSettled
            // to prevent excessive memory allocation from upfront closures. Default to unlimited concurrency (len)
            // if no limit is specified to preserve original behavior.
            const len = requestFns.length;
            const results = new Array(len);
            let currentIndex = 0;
            const concurrencyLimit = options.concurrency || len;

            const worker = async () => {
                while (currentIndex < len) {
                    const index = currentIndex++;
                    try {
                        // Create per-request options without the overhead of deep cloning
