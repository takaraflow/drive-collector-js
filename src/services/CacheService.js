...
class CacheService {
    ...
    async preheat(keys = []) {
        ...
        // ⚡ Bolt Optimization: Replaced chunked array.slice() + Promise.allSettled() with a continuous native async worker pool.
        // Expected impact: Eliminates head-of-line blocking where fast tasks wait for slow tasks in the same batch, and reduces memory overhead from closures, maximizing throughput.
        const concurrency = 3;
        let currentIndex = 0;
        ...
        const worker = async () => {
            while (currentIndex < keys.length) {
                ...
                try {
                    const success = await this._preheatKey(keys[index]);
                    if (success) {
                        results.success++;
                    } else {
                        results.failed++;
    ...
    async batchOperation(operations) {
        ...
        // ⚡ Bolt Optimization: Use native async worker pool instead of unbounded Promise.all
        // This prevents connection pool exhaustion and reduces memory footprint for large batches
        ...
        const workers = Array.from({ length: Math.min(concurrencyLimit, operations.length) }, worker);
        await Promise.all(workers);
