export const STRINGS = {
    ...
    success_with_session: "✅ <b>Proton Drive 绑定成功！</b>\n\n账号: <code>{{username}}</code>\n\n登录会话已保存，后续转存不需要再输入 2FA 验证码。\n现在可以直接发送文件或链接开始转存。",
    success_without_session: "✅ <b>Proton Drive 绑定成功！</b>\n\n账号: <code>{{username}}</code>\n\n当前账号无需 2FA。若后续登录异常，请重新绑定。\n现在可以直接发送文件或链接开始转存。",
    fail_login: "⚠️ <b>Proton Drive 登录或配置验证失败</b>\n\n请检查用户名、密码、2FA 验证码，以及是否误填了 Mailbox Password。",
    fail_session_bootstrap: "⚠️ <b>登录成功，但未能保存长期会话</b>\n\n为避免后续转存再次要求 2FA，请重试绑定。\n若持续失败，可在高级步骤填写 OTP Secret（仅可信自托管环境），或换用其他网盘。",
